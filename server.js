...
getDocs(collection(db, "Request")).then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
        ...
        <div class="input-group">
            <input type="date" class="form-control icon-input dateIssuedInput" ${data.dateIssued ? `value="${data.dateIssued}"` : ''} data-doc-id="${doc.id}" ${data.dateIssued ? '' : 'disabled'}>
        </div>`;
        ...
        <select class="form-select statusSelect" data-doc-id="${doc.id}">
            ...
            <option value="Signature" ${data.status === 'Signature' ? 'selected' : ''}>Signature</option>
            <option value="Release" ${data.status === 'Release' ? 'selected' : ''}>Release</option>
